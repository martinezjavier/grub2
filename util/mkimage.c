			     ...
			     const char *sbat_path)
{
  ...
  switch (image_target->id)
	...
	PE_OHDR (o32, o64, data_size) = grub_cpu_to_le32 (scn_size +
	...
	if (sbat_path)
	  {
	    ...
	    pe_sbat = pe_img + raw_data + hdr_size;
	    grub_util_load_image (sbat_path, pe_sbat);

	    PE_OHDR (o32, o64, resource_table.rva) = grub_host_to_target32 (raw_data);
	    PE_OHDR (o32, o64, resource_table.size) = grub_host_to_target32 (scn_size);
